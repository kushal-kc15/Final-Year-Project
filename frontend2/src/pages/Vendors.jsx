...
const roleLabel = (role) => {
  ...
  if (normalized === 'OWNER') return 'Owner vendor tracking';
  if (normalized === 'STAFF') return 'Staff vendor view';
  return 'Workspace vendor tracking';
};
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-3 sm:px-6 sm:py-4 lg:px-10">
      <PageHeader
        ...
        title="Vendors"
        lede="Review vendor spending and activity."
        actions={
      ...
      {!loading && !loadError && normalizedRows.length > 0 && (
        <section className="mt-4 border-t border-rule pt-3" aria-label="Vendor summary">
      ...
      <section className="mt-3 border-y border-rule py-2.5" aria-label="Vendor search">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-12 md:items-end">
          ...
          <div className="md:col-span-6 flex items-end justify-between gap-3">
            <p className="pb-2 text-xs text-ink-muted">{filtered.length} of {normalizedRows.length} shown</p>
            {hasSearch && (
