...
export default function OrgSetup() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await api.post(`/organizations/${res.data.id}/switch/`).catch(() => null);
      applyAuth({
        user: me.data,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-paper">
      <header className="px-4 py-5 sm:px-10 flex flex-wrap items-center justify-between gap-3">
        <Logo size={28} withWordmark wordmarkSize="lg" />
        <p className="text-xs text-ink-muted">Step 1 of 1 — set up your workspace</p>
      </header>
        ...
        <div className="w-full max-w-lg">
          <div className="text-center">
            ...
            <h1 className="mt-4 font-display text-3xl sm:text-4xl font-medium text-ink leading-tight tracking-tight">
              ...
              <p className="mt-2 text-sm text-ink-muted">
                This workspace will hold your books. The workspace name and optional
                industry description are saved now. Team size, country, and currency
                are local setup preferences for your first view.
              </p>
          </div>
          ...
          <form onSubmit={submit} className="mt-8 space-y-5" noValidate>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Select label="Team size" value={form.size} onChange={update('size')}>
                  <option value="SOLO">Just me</option>
                  <option value="SMALL">2 – 10</option>
                  <option value="MEDIUM">11 – 50</option>
                  <option value="LARGE">50+</option>
                </Select>
                <Select label="Default currency" value={form.currency} onChange={update('currency')}>
                  {CURRENCIES.map((c) => (
